...
if ! $SUPABASE_CMD start -x studio,imgproxy,logflare,vector,edge-runtime 2>&1 | tee artifacts/rag/final-integration-validation/logs/supabase-start.log; then
  echo "Retrying full Supabase start..."
  $SUPABASE_CMD start 2>&1 | tee artifacts/rag/final-integration-validation/logs/supabase-start-retry.log
fi
