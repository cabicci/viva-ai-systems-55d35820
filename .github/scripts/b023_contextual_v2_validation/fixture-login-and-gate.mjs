...
import {
  ...
  requireValue,
  sha256File,
} from "./local-guards.mjs";

const GATE_SHA =
  process.env.B023_GATE_SHA256 ||
  "adc7d2add0d0951a7349300144ce2b4aa7b4ef9a5c3c6c7e1b5cc474932a8c49";
const EXPECTED_CASES = Number(process.env.B023_EXPECTED_GATE_CASES || "16");
const GATE_RECEIPT_NAME = process.env.B023_GATE_RECEIPT_NAME || "browser-network-gate.json";
if (!/^[a-f0-9]{64}$/.test(GATE_SHA)) throw new Error("Gate SHA-256 must be exact lowercase hex");
if (!Number.isInteger(EXPECTED_CASES) || EXPECTED_CASES < 1)
  throw new Error("Expected gate case count must be a positive integer");
if (!/^[a-z0-9][a-z0-9.-]*\.json$/.test(GATE_RECEIPT_NAME))
  throw new Error("Gate receipt name is invalid");
...
if (
  ...
  buildReceipt.head !== head ||
  buildReceipt.localSupabaseOrigin !== localSupabase
) {
  throw new Error("Successful matching local build/module-graph receipt required");
}
const manifest = join(
  repo,
  "src/lib/lesson-visuals/contextual-v2/runtime/contextualV2BrowserManifest.json",
);
if (sha256File(manifest) !== buildReceipt.browserManifestSha256)
  throw new Error("Built manifest identity changed");
...
async function localAdminRequest(path, method, payload) {
  ...
  if (!response.ok)
    throw new Error(
      `Local fixture operation ${method} ${url.pathname} failed (HTTP ${response.status})`,
    );
  ...
  return new Promise((resolveGate, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0)
        return reject(
          new Error(
            `Frozen browser gate failed (exit ${code}); inspect local sanitized diagnostics`,
          ),
        );
      try {
        ...
        if (
          result.result !== "PASS" ||
          result.candidateGitHead !== head ||
          result.totals?.cases !== EXPECTED_CASES ||
          result.gateScriptSha256 !== GATE_SHA ||
          ("browserManifestSha256" in result &&
            result.browserManifestSha256 !== buildReceipt.browserManifestSha256)
        ) {
          throw new Error("Frozen gate receipt identity/count mismatch");
        }
        writeFileSync(join(out, GATE_RECEIPT_NAME), `${JSON.stringify(result, null, 2)}\n`);
        resolveGate(result);
      } catch (error) {
        reject(error);
      }
...
try {
  ...
  if (!tokenResponse.ok() || (await tokenResponse.json()).user?.id !== userId)
  ...
  const childEnv = {
    ...process.env,
    B023_AUTH_STORAGE_STATE: statePath,
    B023_DISPOSABLE_FIXTURE_ID: fixtureId,
  };
  ...
  if (userId) {
    try {
      await localAdminRequest(`/auth/v1/admin/users/${userId}`, "DELETE");
    } catch (error) {
      cleanupError = error;
    }
  }
