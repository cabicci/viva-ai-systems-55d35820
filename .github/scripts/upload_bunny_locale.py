#!/usr/bin/env python3
"""Locale-aware Bunny Stream uploader for the 6-cell pilot.

Reads the produced MP4 for (LID, LOCALE), uploads it to Bunny Stream, and
inserts/replaces the composite key `${LID}__${LOCALE}` in
src/lib/bunny-videos.ts so getBunnyEmbedUrlForLocale() resolves it at runtime.

Env:
  LID                        Lesson id
  LOCALE                     Locale code (ar-MSA | ar-Gulf | en)
  MP4_PATH                   Absolute or repo-relative MP4 path
  BUNNY_STREAM_API_KEY       Bunny Stream API access key
  BUNNY_STREAM_LIBRARY_ID    Bunny Stream library id (numeric)
"""
from __future__ import annotations
import json
import os
import re
import sys
import urllib.request
import urllib.error

LID = os.environ["LID"]
LOCALE = os.environ["LOCALE"]
MP4_PATH = os.environ.get("MP4_PATH") or f"public/lessons/intro/{LID}__{LOCALE}.mp4"
API_KEY = os.environ["BUNNY_STREAM_API_KEY"]
LIBRARY_ID = os.environ["BUNNY_STREAM_LIBRARY_ID"]

REGISTRY_PATH = "src/lib/bunny-videos.ts"
BASE = f"https://video.bunnycdn.com/library/{LIBRARY_ID}/videos"
COMPOSITE_KEY = f"{LID}__{LOCALE}"
GUID_OUT = f"/tmp/bunny-guid-{COMPOSITE_KEY}.txt"


def _req(method: str, url: str, *, body: bytes | None = None, ctype: str | None = None) -> bytes:
    req = urllib.request.Request(url, method=method, data=body)
    req.add_header("AccessKey", API_KEY)
    req.add_header("accept", "application/json")
    if ctype:
        req.add_header("content-type", ctype)
    try:
        with urllib.request.urlopen(req, timeout=600) as resp:
            return resp.read()
    except urllib.error.HTTPError as e:
        sys.stderr.write(f"HTTP {e.code} {method} {url}: {e.read().decode(errors='ignore')}\n")
        raise


def create_video(title: str) -> str:
    raw = _req("POST", BASE, body=json.dumps({"title": title}).encode(), ctype="application/json")
    data = json.loads(raw.decode())
    guid = data.get("guid")
    if not guid:
        raise RuntimeError(f"No guid in create response: {data}")
    return guid


def upload_mp4(guid: str, mp4_path: str) -> None:
    with open(mp4_path, "rb") as f:
        data = f.read()
    _req("PUT", f"{BASE}/{guid}", body=data, ctype="application/octet-stream")


def delete_video(guid: str) -> None:
    try:
        _req("DELETE", f"{BASE}/{guid}")
    except Exception as e:
        sys.stderr.write(f"(warn) failed to delete old bunny video {guid}: {e}\n")


def verify_playable(guid: str) -> str:
    """Fetch video metadata; return playback iframe URL."""
    raw = _req("GET", f"{BASE}/{guid}")
    meta = json.loads(raw.decode())
    status = meta.get("status")
    sys.stderr.write(f"[bunny] guid={guid} status={status} length={meta.get('length')}\n")
    return f"https://iframe.mediadelivery.net/embed/{LIBRARY_ID}/{guid}?autoplay=false&preload=true"


def update_registry(key: str, guid: str) -> str | None:
    src = open(REGISTRY_PATH).read()
    entry = f'  "{key}": "{guid}",'
    pattern = re.compile(rf'^(\s*)"{re.escape(key)}":\s*"([^"]+)",\s*$', re.MULTILINE)
    m = pattern.search(src)
    prev: str | None = None
    if m:
        prev = m.group(2)
        if prev == guid:
            return None
        src = pattern.sub(entry, src, count=1)
    else:
        block_re = re.compile(
            r'(export const BUNNY_VIDEO_GUIDS:\s*Record<string,\s*string>\s*=\s*\{)([\s\S]*?)(\n\};)'
        )
        bm = block_re.search(src)
        if not bm:
            raise RuntimeError("Could not locate BUNNY_VIDEO_GUIDS block in registry")
        head, body, tail = bm.group(1), bm.group(2), bm.group(3)
        lines = [ln for ln in body.split("\n") if ln.strip()]
        entry_re = re.compile(r'^\s*"([^"]+)":\s*"[^"]+",\s*$')
        kv: list[tuple[str, str]] = []
        other: list[str] = []
        for ln in lines:
            mm = entry_re.match(ln)
            if mm:
                kv.append((mm.group(1), ln))
            else:
                other.append(ln)
        kv.append((key, entry))
        kv.sort(key=lambda x: x[0])
        new_body = "\n" + "\n".join([*other, *(ln for _, ln in kv)]) + "\n"
        src = src[: bm.start()] + head + new_body + tail + src[bm.end():]
    open(REGISTRY_PATH, "w").write(src)
    return prev


def main() -> int:
    # Idempotent: reuse existing guid for the same (LID, LOCALE) if present.
    if os.path.isfile(GUID_OUT):
        guid = open(GUID_OUT).read().strip()
        if guid:
            print(f"[bunny] reusing guid {guid} for {COMPOSITE_KEY}", flush=True)
            update_registry(COMPOSITE_KEY, guid)
            print(f"::notice::playback_url={verify_playable(guid)}")
            return 0
    if not os.path.isfile(MP4_PATH):
        sys.stderr.write(f"MP4 not found at {MP4_PATH}\n")
        return 1
    title = f"{LID} [{LOCALE}]"
    print(f"[bunny] creating video for {COMPOSITE_KEY} (title: {title!r})", flush=True)
    guid = create_video(title)
    print(f"[bunny] created guid={guid}, uploading {MP4_PATH} ...", flush=True)
    try:
        upload_mp4(guid, MP4_PATH)
    except Exception:
        delete_video(guid)
        raise
    print(f"[bunny] upload complete; updating {REGISTRY_PATH}", flush=True)
    prev = update_registry(COMPOSITE_KEY, guid)
    if prev and prev != guid:
        print(f"[bunny] replaced previous guid {prev} for {COMPOSITE_KEY}; deleting old video", flush=True)
        delete_video(prev)
    with open(GUID_OUT, "w") as f:
        f.write(guid)
    url = verify_playable(guid)
    print(f"::notice::lesson={LID} locale={LOCALE} guid={guid}")
    print(f"::notice::playback_url={url}")
    print(f"[bunny] done: {COMPOSITE_KEY} -> {guid}", flush=True)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
