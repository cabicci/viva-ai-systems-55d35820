...
function psql(sql: string): string {
  ...
  const tmp = `/tmp/rag-paid-${Date.now()}-${Math.random().toString(16).slice(2)}.sql`;
  // Write SQL via docker stdin to avoid shell escaping issues with vectors
...
`);
  ...
  for (let i = 0; i < successChunks.length; i += insertBatchSize) {
    const slice = successChunks.slice(i, i + insertBatchSize);
    const values = slice
      .map((c) => {
...
`);
  ...
  // Isolation checks on staging rows
