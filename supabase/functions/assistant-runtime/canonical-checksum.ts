/**
 * Deno-importable twin of src/lib/rag/canonical-checksum.ts.
 * Keep byte-for-byte digest parity with the Node/shared implementation
 * (proven by src/lib/__tests__/rag-canonical-checksum.test.ts).
 *
 * Shared canonical contract:
 * - UTF-8; CRLF→LF; strip trailing horizontal whitespace before newlines
 * - collapse \\n{3,} → \\n\\n; trim; JSON sorted keys; arrays preserve order
 * - SHA-256 digest = exactly 64 lowercase hex chars
 */
...
export function sha256CanonicalHex(text: string): string {
  return sha256Utf8Bytes(new TextEncoder().encode(normalizeCanonicalText(text)));
}
