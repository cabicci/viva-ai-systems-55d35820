-- DB-C1: enforce caller identity on consume_rate_limit.
-- service_role (trusted server/edge) may consume any p_user_id bucket.
-- Any other role (if EXECUTE is ever granted) must match auth.uid() = p_user_id.
-- Does not change EXECUTE grants or table policies (DB-C2 untouched).
