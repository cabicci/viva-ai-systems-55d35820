...
import { coordinateCheckout, type CheckoutIntent, type CheckoutSession } from "../../../../supabase/functions/_shared/stripe-checkout-intent";

const subscription = (price = "price_plus", latest = "in_current") => ({
  id: "sub_same", status: "active", latest_invoice: latest,
  items: { data: [{ price: { id: price } }] },
});
const line = (price: string, amount: number, sub = "sub_same") => ({
  amount, pricing: { price_details: { price } },
  parent: { subscription_item_details: { subscription: sub } },
});
const invoice = (lines = [line("price_plus", 14000)]) => ({
  id: "in_current", status: "paid", amount_paid: 14000, amount_remaining: 0,
  parent: { subscription_details: { subscription: "sub_same" } },
  lines: { data: lines, has_more: false },
});
const decide = (object: Record<string, any>, sub = subscription()) =>
...
describe("Paid plan evidence used by the webhook", () => {
  it("accepts current API proration debit and ignores unused-time credit", () => {
    expect(decide(invoice([line("price_pro", -9000), line("price_plus", 23000)])))
      .toMatchObject({ valid: true, priceId: "price_plus", transition: "payment_succeeded" });
  });
  it("never grants an unpaid Plus from a delayed paid Pro invoice", () => {
    expect(decide(invoice([line("price_pro", 16900)]))).toMatchObject({ valid: false, error: "PAID_PLAN_EVIDENCE_MISMATCH" });
  });
  it("rejects an older invoice even if the price happens to match", () => {
    expect(decide({ ...invoice(), id: "in_old" })).toMatchObject({ valid: false, error: "PAID_INVOICE_MISMATCH" });
  });
  it("rejects unpaid and pending updates", () => {
    expect(decide({ ...invoice(), status: "open" }).valid).toBe(false);
    expect(decidePaidPlanEvidence({ eventType: "invoice.paid", object: invoice(),
      subscription: { ...subscription(), pending_update: { expires_at: 123 } } }).valid).toBe(false);
  });
  it("rejects unrelated, manual, ambiguous and incomplete invoice lines", () => {
    ...
    expect(decide(invoice([{ amount: 14000, pricing: { price_details: { price: "price_plus" } } } as any])).valid).toBe(false);
    expect(decide(invoice([line("price_pro", 16900), line("price_plus", 30900)])).valid).toBe(false);
    expect(decide({ ...invoice(), lines: { data: [line("price_plus", 14000)], has_more: true } }).valid).toBe(false);
  });
  ...
  it("handles legacy line shapes as well as current ones", () => {
    expect(decide(invoice([{ amount: 14000, price: { id: "price_plus" }, subscription: "sub_same" } as any])).valid).toBe(true);
  });
  it("binds Checkout to its actual line price and latest invoice", () => {
    const object = { id: "cs_paid", invoice: "in_current", subscription: "sub_same", payment_status: "paid" };
    const evidence = (price: string, obj = object) => decidePaidPlanEvidence({
      eventType: "checkout.session.completed", object: obj, subscription: subscription(),
...
function harness() {
  ...
  const deps = {
    prepare: async () => {
      if (!current) current = { subscription_id: "internal", checkout_generation: String(++sequence),
        checkout_session_id: null, selection_matches: true, selection: selected };
    ...
    attach: async (intent: CheckoutIntent, id: string) => {
      ...
      current.checkout_session_id = id; return true;
    },
    close: async (intent: CheckoutIntent) => {
      if (current?.checkout_generation !== intent.checkout_generation || current.checkout_session_id !== intent.checkout_session_id) return false;
      current = null; return true;
    },
    confirm: async (intent: CheckoutIntent) => current?.checkout_generation === intent.checkout_generation,
  };
  return { deps, calls, sessions, choose: (value: string) => { selected = value; },
    pause: (value: () => Promise<void>) => { pauseCreate = value; },
    failExpiration: () => { failExpiration = true; } };
}
...
describe("Actual checkout coordinator", () => {
  it("expires before replacing Pro → Plus → Pro; repeats reuse one session", async () => {
    ...
    expect(h.calls).toEqual(["create:cs_1", "expire:cs_1", "create:cs_2", "expire:cs_2", "create:cs_3"]);
  ...
  it("does not supersede an in-flight creation for a different selection", async () => {
    ...
    h.pause(() => { started(); return new Promise<void>(r => { resume = r; }); });
  ...
  it("never creates a replacement if payment wins the expiration race", async () => {
    ...
    h.choose("plus"); h.failExpiration();
