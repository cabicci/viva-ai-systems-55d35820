...
import { buildEntitlementSnapshot, evaluateAccess } from "@/lib/billing";
...
const ENTITLEMENT_EDGE = "supabase/functions/billing-entitlement/index.ts";
...
describe("billing launch closure SQL hardening", () => {
  ...
  it("enforces quota before reserve_ai_quota insert", () => {
    ...
    expect(closureSql).toMatch(/reserve_ai_quota[\s\S]*p_units > v_remaining/);
  });
  ...
  it("routes entitlement edge resource checks through evaluate_access RPC", () => {
    const edge = readRepoFile(ENTITLEMENT_EDGE);
    expect(edge).toContain("/rest/v1/rpc/evaluate_access");
    expect(edge).toContain("p_resource_type");
    expect(edge).toMatch(/Fail closed|fail closed|decision\?\.allowed === true/);
    expect(edge).not.toMatch(/if \(entitled && resourceType === "lesson" && resourceId\)/);
  });
});
