...
function fixture() {
  // Test fixtures intentionally allow mutations across several Stripe object shapes.
  ...
  const objects: Record<string, any> = {
    ...
    "/subscriptions/sub_test": {
      id: "sub_test",
      livemode: false,
      customer: "cus_test",
      latest_invoice: "in_test",
      metadata: {
        internal_subscription_id: "internal",
        user_id: "owner",
...
describe("Stripe refund authoritative invoice correlation", () => {
  it("uses provider relations and current refund status, not supplied metadata", async () => {
    ...
    expect((await resolveStripeRefund("re_test", get))?.rpc).toMatchObject({
      p_gateway_invoice_id: "in_test",
      p_amount_minor: 5000,
      p_is_latest_invoice: true,
      p_status: "succeeded",
    });
  });
  ...
  it.each(["refund", "invoice", "subscription", "intent"])("rejects Live %s data", async (kind) => {
    const { objects, get } = fixture();
    const paths = {
      refund: "/refunds/re_test",
      invoice: "/invoices/in_test",
      subscription: "/subscriptions/sub_test",
      intent: "/payment_intents/pi_test",
    };
  ...
  it("ignores unrelated non-invoice payments", async () => {
    const { objects, get } = fixture();
    objects.payments.data = [];
    expect(await resolveStripeRefund("re_test", get)).toBeNull();
  });
  it.each([false, true])(
    "cancels only after the database proves a current full refund: %s",
    async (cancel) => {
      const { get } = fixture();
      ...
      vi.stubGlobal(
        ...
        vi.fn(async (url: string, options: RequestInit) => {
          ...
          return new Response(
            JSON.stringify(await get(url.replace("https://api.stripe.com/v1", ""))),
          );
        }),
      );
      ...
      expect(deleted).toEqual(
        cancel
          ? ["https://api.stripe.com/v1/subscriptions/sub_test?invoice_now=false&prorate=false"]
          : [],
      );
    },
  );
