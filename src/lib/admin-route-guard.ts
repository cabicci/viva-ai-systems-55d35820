...
export function decideAdminBeforeLoad(
  outcome: { status: "authorized"; isAdmin: boolean } | { status: "unauthorized" },
 ...
 */
export async function loadProtectedAdminPayloadIfAuthorized<T>(load: () => Promise<T>): Promise<T> {
  let outcome: { status: "authorized"; isAdmin: boolean } | { status: "unauthorized" };
  try {
    const res = await assertAdminAccess();
