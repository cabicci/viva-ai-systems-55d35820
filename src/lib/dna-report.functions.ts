...
export const generateDnaReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    push(`\n> **Lesson ID convention:** \`{pathId}-{moduleSlug}-{lessonSlug}\` (مثال: \`automator-m1-systems-view\`). الـ id لوحده بيشفّر path + module + ترتيب — متخترعش رقم درس عام زي "الدرس 22".\n`);
