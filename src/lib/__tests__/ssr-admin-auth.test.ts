...
const EXPIRED_SHAPED_JWT = "eyJhbGciOiJub25lIn0.eyJleHAiOjF9.sig";

function headerMap(init: Record<string, string | null>) {
  return {
    get(name: string) {
      const key = Object.keys(init).find((k) => k.toLowerCase() === name.toLowerCase());
...
describe("access token cookie / header extraction", () => {
  ...
  it("treats compact but unverifiable-shaped tokens as extractable only; role still requires getClaims", () => {
    expect(isCompactJwt(EXPIRED_SHAPED_JWT)).toBe(true);
    const headers = headerMap({
      authorization: `Bearer ${EXPIRED_SHAPED_JWT}`,
    });
    expect(extractAccessTokenFromHeaders(headers)).toBe(EXPIRED_SHAPED_JWT);
  });

  it("writes and clears document cookie with SameSite=Lax", () => {
...
describe("admin-route-guard SSR contract (static)", () => {
  const guardSrc = readFileSync(path.join(REPO_ROOT, "src/lib/admin-route-guard.ts"), "utf8");
  const adminFnsSrc = readFileSync(path.join(REPO_ROOT, "src/lib/admin.functions.ts"), "utf8");
  const serverAuthSrc = readFileSync(
    path.join(REPO_ROOT, "src/lib/ssr-request-auth.server.ts"),
    "utf8",
  );
  const authCtxSrc = readFileSync(path.join(REPO_ROOT, "src/lib/auth-context.tsx"), "utf8");

  it("does not skip authorization on SSR via window check", () => {
    expect(guardSrc).not.toMatch(/typeof window === ["']undefined["']\s*\)\s*return/);
  ...
  it("server resolver uses getClaims and never trusts client role metadata", () => {
  ...
  it("does not log tokens, cookies, or authorization headers", () => {
    for (const src of [serverAuthSrc, guardSrc, authCtxSrc, adminFnsSrc]) {
      expect(src).not.toMatch(/console\.(log|info|debug|warn|error).*token/i);
      expect(src).not.toMatch(/console\.(log|info|debug|warn|error).*cookie/i);
      expect(src).not.toMatch(/console\.(log|info|debug|warn|error).*authorization/i);
      expect(src).not.toMatch(/captureError\([^)]*access_token/i);
      expect(src).not.toMatch(/captureWarn\([^)]*access_token/i);
    }
  });
  ...
  it("auth-context syncs access-token cookie without exposing it in UI state", () => {
    expect(authCtxSrc).toContain("syncAccessTokenCookie");
    expect(authCtxSrc).not.toMatch(/accessTokenCookie|masaarat_access_token/);
  });
});

describe("decideAdminBeforeLoad fail-closed decisions", () => {
  it("denies missing identity (SSR unauthenticated)", () => {
  ...
  it("denies invalid or unverifiable identity", () => {
    expect(decideAdminBeforeLoad({ status: "unauthorized" })).toEqual({
      type: "redirect",
      to: "/login",
    });
  ...
  it("denies authenticated non-admin", () => {
    expect(decideAdminBeforeLoad({ status: "authorized", isAdmin: false })).toEqual({
      type: "redirect",
      to: "/dashboard",
    });
  });
  ...
  it("allows authorized admin", () => {
    expect(decideAdminBeforeLoad({ status: "authorized", isAdmin: true })).toEqual({
      type: "allow",
    });
  });

  it("ignores client-provided role/state — only server outcome matters", () => {
    const clientClaims = { role: "admin", isAdmin: true };
    void clientClaims;
    expect(decideAdminBeforeLoad({ status: "authorized", isAdmin: false })).toEqual({
      type: "redirect",
      to: "/dashboard",
...
describe("protected loader / action denial boundaries", () => {
  it("does not return protected payload when identity missing", async () => {
    ...
    vi.spyOn(await import("@/lib/admin.functions"), "assertAdminAccess").mockImplementation(
      async () => {
  ...
  it("does not return protected payload for non-admin", async () => {
    const load = vi.fn(async () => ({ secret: "admin-data" }));
    vi.spyOn(await import("@/lib/admin.functions"), "assertAdminAccess").mockResolvedValue({
      isAdmin: false,
      userId: "user-1",
    });
  ...
  it("does not execute side effects when unauthorized", async () => {
    ...
    await expect(runProtectedAdminActionIfAuthorized(sideEffect)).rejects.toThrow(/Unauthorized/);
  ...
  it("does not execute side effects for non-admin", async () => {
    ...
    await expect(runProtectedAdminActionIfAuthorized(sideEffect)).rejects.toThrow(
...
describe("client navigation defense in depth remains available", () => {
  it("AdminGate still exists as client-side secondary gate", () => {
    const gateSrc = readFileSync(path.join(REPO_ROOT, "src/components/AdminGate.tsx"), "utf8");
  ...
  it("Admin Continuity surfaces remain on requireAdminBeforeLoad without route edits", () => {
    const adminIndex = readFileSync(path.join(REPO_ROOT, "src/routes/admin.index.tsx"), "utf8");
    expect(adminIndex).toContain("beforeLoad: requireAdminBeforeLoad");
    expect(adminIndex).toContain('createFileRoute("/admin/")');
  });
});
