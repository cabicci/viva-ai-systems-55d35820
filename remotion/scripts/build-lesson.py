...
def write_scenes_module(lesson_id, scenes, frames, locale=None):
    ...
    visuals = []
    for s in scenes:
        visual = {"card": s["card"], "accent": s["accent"], **s["visual"]}
        # Stamp locale only for localized generation. Legacy Egyptian modules
        # omit the field so Remotion keeps its historical Arabic/RTL chrome.
        if locale:
    ...
    if locale:
        locale_export = (
            f"\nexport const LOCALE = {json.dumps(locale)} as const;\n"
        )
    body = (
    ...
    with open(os.path.join(out_dir, f"{lesson_id}.gen.ts"), "w", encoding="utf-8") as f:
        f.write(body)
