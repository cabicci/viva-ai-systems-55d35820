...
const {course,images,media,level1Media,localMedia}=payload;
...
function Lesson({d,n,w,locale}:{d:any,n:number,w:any,locale:string}){
 ...
 {localMedia&&(n===1||level1Media?.[String(n)]?.[locale]?.url)?card(w.video,<video controls playsInline preload="metadata" className="w-full aspect-video rounded-xl border border-border" src={n===1?media[locale].url:level1Media[String(n)][locale].url}>{(n===1?media[locale].captions:level1Media[String(n)][locale].captions)&&<track kind="captions" src={n===1?media[locale].captions:level1Media[String(n)][locale].captions} srcLang={locale==='en'?'en':'ar'} label={locale}/>}</video>,PlayCircle,'primary','watch'):<p className="text-center text-xs text-muted-foreground py-2">{n===1||level1Media?.[String(n)]?.[locale]?.url?w.local:w.missing}</p>}
 ...
 {card(w.image,<figure className="overflow-hidden rounded-2xl border border-primary/20 bg-card"><img data-lesson-image src={n===1?images[locale]:images["lesson-"+String(n).padStart(2,"0")][locale]} alt={d.pageIllustration?.title||d.imageBrief.altText} className="block w-full h-auto"/></figure>,BookOpen,'primary')}
