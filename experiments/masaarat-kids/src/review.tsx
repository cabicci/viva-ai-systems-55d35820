...
const {courses,images,media,level1Media,advancedMedia,localMedia}=payload;
const locales=['ar-EG','ar-MSA','ar-Gulf','en'];
const names=['المصرية','العربية الفصحى','الخليجية','English'];
const levelNames={ar:[['أكتشف وأستخدم','المستوى الأول · ١٠–١٢ سنة'],['أبحث وأطوّر','المستوى الثاني · ١٢–١٤ سنة'],['أصمّم وأقيّم','المستوى الثالث · ١٤–١٦ سنة']],en:[['Discover & use','Level 1 · Ages 10–12'],['Research & improve','Level 2 · Ages 12–14'],['Design & evaluate','Level 3 · Ages 14–16']]};
const mediaItem=(level:number,n:number,locale:string)=>level===1?(n===1?media[locale]:level1Media?.[String(n)]?.[locale]):advancedMedia?.[String(level)]?.[String(n)]?.[locale];
...
function Lesson({d,n,level,w,locale}:{d:any,n:number,level:number,w:any,locale:string}){
 const [answer,setAnswer]=useState(''),[parts,setParts]=useState(['','','']),[built,setBuilt]=useState('');
 const lessonMedia=mediaItem(level,n,locale);
 let index=0;
 ...
 {localMedia&&lessonMedia?.url?card(w.video,<video controls playsInline preload="metadata" className="w-full aspect-video rounded-xl border border-border" src={lessonMedia.url}>{lessonMedia.captions&&<track kind="captions" src={lessonMedia.captions} srcLang={locale==='en'?'en':'ar'} label={locale}/>}</video>,PlayCircle,'primary','watch'):<p className="text-center text-xs text-muted-foreground py-2">{lessonMedia?.url?w.local:w.missing}</p>}
 ...
 {card(w.image,<figure className="overflow-hidden rounded-2xl border border-primary/20 bg-card"><img data-lesson-image src={level===1?(n===1?images[locale]:images["lesson-"+String(n).padStart(2,"0")][locale]):images["level-"+level]["lesson-"+String(n).padStart(2,"0")][locale]} alt={d.pageIllustration?.title||d.imageBrief.altText} className="block w-full h-auto"/></figure>,BookOpen,'primary')}
 ...
 {card(w.quiz,<QuizBlock lessonId={'kids-preview-l'+level+'-'+n} items={d.quiz.map((q:any,i:number)=>({id:q.id||'q'+i,bloom:['remember','understand','apply'][Math.min(i,2)],question:q.question,options:q.options,correctIndex:q.answer,explanation:q.explanation}))}/>,CheckCircle2)}
...
function readRoute(){
 const lesson=location.hash.match(/^#\/level\/(\d+)\/learn\/(\d+)$/);
 if(lesson){const level=Number(lesson[1]),index=Number(lesson[2])-1;if(courses[String(level)]&&index>=0&&index<courses[String(level)].length)return {level,selected:index};}
 const page=location.hash.match(/^#\/level\/(\d+)$/);
 if(page&&courses[page[1]])return {level:Number(page[1]),selected:null};
 const legacy=location.hash.match(/^#\/learn\/(\d+)$/);
 if(legacy&&Number(legacy[1])>=1&&Number(legacy[1])<=courses['1'].length)return {level:1,selected:Number(legacy[1])-1};
 if(location.hash==='#/dashboard')return {level:1,selected:null};
 return {level:null,selected:null};
}
function App(){
 const [locale,setLocale]=useState('ar-EG'),[route,setRoute]=useState(readRoute),[expanded,setExpanded]=useState(true);
 const {level,selected}=route,course=level?courses[String(level)]:null;
 const w={...labels[locale==='en'?'en':'ar'],level:level?levelNames[locale==='en'?'en':'ar'][level-1][1]:'',...(locale==='ar-MSA'?{goals:'ماذا ستتعلّم؟',missing:'يمكنك البدء بالشرح المكتوب. فيديو الدرس قيد الإعداد.'}:locale==='ar-Gulf'?{goals:'وش بتتعلّم؟',missing:'تقدر تبدأ بالشرح المكتوب. فيديو الدرس قيد التجهيز.'}:{})};
 ...
 useEffect(()=>{document.documentElement.lang=locale==='en'?'en':'ar';document.documentElement.dir=locale==='en'?'ltr':'rtl';window.scrollTo(0,0);},[locale,level,selected]);
 const Previous=locale==='en'?ArrowLeft:ArrowRight,Next=locale==='en'?ArrowRight:ArrowLeft;
 const d=selected===null||!course?null:course[selected].locales[locale];
 ...
 <a href="#/levels" aria-label={w.brand} className="inline-flex items-center gap-2.5 self-start"><img src={payload.logo} alt={locale==='en'?'Masaarat':'مسارات'} className="w-28 h-12 object-contain"/><span aria-label="KIDS" dir="ltr" className="inline-flex text-xl sm:text-2xl font-black tracking-[0.04em]" style={{WebkitTextStroke:'0.35px #173c4f'}}><span style={{color:'#9be3c4'}}>K</span><span style={{color:'#3dbbbf'}}>I</span><span style={{color:'#c2acda'}}>D</span><span style={{color:'#8db3e9'}}>S</span></span></a>
 ...
 {level===null?<main className="flex-1 min-w-0 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8 md:py-12" data-levels>
 <header className="mb-8"><h1 className="text-2xl md:text-4xl font-black leading-tight">{w.brand}</h1><p className="mt-3 text-muted-foreground">{locale==='en'?'Choose your level':'اختار المستوى المناسب ليك'}</p></header>
 <div className="grid md:grid-cols-3 gap-4">{[1,2,3].map(n=><a key={n} id={'level-card-'+n} href={'#/level/'+n} className="glass rounded-2xl border border-primary/20 p-5 min-h-48 flex flex-col gap-4 hover:bg-primary/[0.04] focus-visible:outline-primary">
 <span dir="ltr" className="grid h-10 w-12 place-items-center rounded-lg bg-[image:var(--gradient-primary)] text-primary-foreground text-sm font-black">L{n}</span>
 <span className="text-xl font-bold">{levelNames[locale==='en'?'en':'ar'][n-1][0]}</span>
 <span className="text-sm text-muted-foreground">{levelNames[locale==='en'?'en':'ar'][n-1][1]} · {courses[String(n)].length} {w.lessons}</span>
 </a>)}</div></main>:selected===null?<main className="flex-1 min-w-0 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8 md:py-12" data-dashboard>
 <a id="back-levels" href="#/levels" className="inline-flex items-center gap-2 rounded-full glass border border-primary/30 px-3 py-2 text-xs font-medium mb-6"><Previous className="h-4 w-4"/>{locale==='en'?'All levels':'كل المستويات'}</a>
 <header className="mb-8"><h1 className="text-2xl md:text-4xl font-black leading-tight">{levelNames[locale==='en'?'en':'ar'][level-1][0]}</h1><p className="mt-3 text-muted-foreground">{w.level}</p></header>
 ...
 <span dir="ltr" className="grid h-8 min-w-10 px-2 place-items-center rounded-md shrink-0 bg-[image:var(--gradient-primary)] text-primary-foreground text-xs font-black">M{level}</span><span className="flex-1 min-w-0 font-semibold">{w.level}</span><ChevronDown className={'h-4 w-4 text-muted-foreground shrink-0 transition-transform '+(expanded?'rotate-180':'')}/>
 </button>
 ...
 <Button asChild size="sm" variant="hero" className="w-full h-8 text-xs"><a data-open-lesson={i+1} href={'#/level/'+level+'/learn/'+(i+1)}><Play className="h-3 w-3"/>{t('dashboard.lesson.start')}</a></Button>
 </div>)}</div></div>}
 </div></div></main>:<main className="flex-1 min-w-0 max-w-[48rem] mx-auto w-full px-4 sm:px-6 py-8 md:py-12">
 <a id="back-dashboard" href={'#/level/'+level} className="inline-flex items-center gap-2 rounded-full glass border border-primary/30 px-3 py-2 text-xs font-medium text-foreground/90 hover:bg-foreground/5 transition mb-6"><Previous className="h-4 w-4"/>{t('learn.backToDashboard')}</a>
 ...
 <Lesson key={level+locale+selected} d={d} n={selected+1} level={level} w={w} locale={locale}/>
 ...
 <div className="flex min-w-0 flex-wrap gap-2">{selected>0?<Button asChild variant="glass" size="sm"><a id="prev" href={'#/level/'+level+'/learn/'+selected}><Previous className="h-4 w-4"/>{t('learn.nav.previous')}</a></Button>:<span/>}</div>
 <div className="flex gap-2"><Button asChild variant="violet" size="sm"><a id={selected<11?'next':'finish-path'} href={selected<11?'#/level/'+level+'/learn/'+(selected+2):'#/level/'+level}>{t(selected<11?'learn.nav.next':'learn.backToDashboard')}<Next className="h-4 w-4"/></a></Button></div></nav>
