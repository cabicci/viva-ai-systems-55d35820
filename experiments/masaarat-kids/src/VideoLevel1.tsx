...
type Props={lesson:number;locale:Locale;timings:Timing[]};
const source=videoData as Record<string,{title:string;scenes:{title:string;display:string;narration:string}[]}>;
...
function Scene({lesson,locale,index}:{lesson:number;locale:Locale;index:number}){
 const frame=useCurrentFrame(),data=source[String(lesson).padStart(2,'0')+'/'+locale],scene=data.scenes[index],rtl=locale!=='en';
...
function Video({lesson,locale,timings}:Props){let cursor=0;return <AbsoluteFill>{timings.map((t,i)=>{const start=cursor;cursor+=t.frames;return <Sequence key={i} from={start} durationInFrames={t.frames}><Scene lesson={lesson} locale={locale} index={i}/>{t.audio&&<Audio src={staticFile(t.audio)}/>}</Sequence>})}</AbsoluteFill>}
function Root(){return <>{Array.from({length:11},(_,i)=>i+2).flatMap(lesson=>(['ar-EG','ar-MSA','ar-Gulf','en'] as Locale[]).map(locale=><Composition key={lesson+'-'+locale} id={'kids-l1-'+String(lesson).padStart(2,'0')+'-'+locale} component={Video} width={1280} height={720} fps={24} durationInFrames={672} defaultProps={{lesson,locale,timings:source[String(lesson).padStart(2,'0')+'/'+locale].scenes.map(()=>({frames:96})),}} calculateMetadata={({props})=>({durationInFrames:props.timings.reduce((s,t)=>s+t.frames,0)})}/>))}</>}
registerRoot(Root);
