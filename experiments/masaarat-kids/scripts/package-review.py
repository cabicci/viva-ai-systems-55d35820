...
shutil.copytree(BASE/"public/generated/stills",out/"generated/stills")
...
shutil.copytree(BASE/"evidence",out/"review-evidence",ignore=shutil.ignore_patterns("*.png","*.log"))
(out/"START_HERE.txt").write_text("MASAARAT KIDS - FIRST LESSON REVIEW\n\nExtract the entire ZIP, then open index.html. Keep the folders beside it.\nSelect Egyptian Arabic, Modern Standard Arabic, Gulf Arabic, or English in the page.\nThe package contains four narrated videos, 48 scene images, quizzes, a prompt-building activity and sourced lesson hints.\nEverything in this review page works locally. The helper uses curated lesson answers; a generative RAG service is not connected.\nAudio transcription review is automated. Human pronunciation/dialect approval and educational approval are still pending.\nNo production release is included.\n",encoding="utf-8")
(out/"package-manifest.json").write_text(json.dumps({"sourceSha":__import__("os").environ.get("GITHUB_SHA"),"files":files,"images":48,"status":"owner-review"},indent=2)+"\n",encoding="utf-8")
assert len(list((out/"generated/stills").glob("*/*.png")))==48
print("Offline package ready: 4 videos, 48 images, lesson interface and review evidence.")
