...
folder = BASE/"public/generated/audio"/locale
timings = json.loads((folder/"timings.json").read_text())
assert len(timings) == len(lesson["scenes"]) == 12
evidence = []
cursor = 0
captions = ["WEBVTT", ""]
def stamp(seconds):
...
for i, (scene, timing) in enumerate(zip(lesson["scenes"], timings)):
    ...
    assert timing["audio"] == f"generated/audio/{locale}/{i:02}.wav"
...
video = BASE/"public/generated/videos"/(locale+".mp4")
...
stills = list((BASE/"public/generated/stills"/locale).glob("*.png"))
assert len(stills)==12, "Missing scene stills"
caption_path = video.with_suffix(".vtt")
caption_path.write_text("\n".join(captions),encoding="utf-8")
result = {"locale":locale,"status":"technical-pass","frames":cursor,"durationSeconds":cursor/24,"videoSha256":hashlib.sha256(video.read_bytes()).hexdigest(),"audioSegments":evidence,"stills":len(stills),"captions":"scene-aligned; source script","spokenAccuracyReview":"pending human listening"}
(BASE/"evidence").mkdir(exist_ok=True)
(BASE/"evidence"/("narrated-"+locale+".json")).write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
print(f"VERIFIED {locale}: 12 audible segments, H264/AAC, {cursor/24:.2f}s, 12 stills")
