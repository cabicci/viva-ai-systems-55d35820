...
for(const locale of ['ar-EG','ar-MSA','ar-Gulf','en']){
 ...
 for(let i=0;i<12;i++){
  ...
  if(i<2){const image=page.locator('[data-lesson-image]');if(!await image.evaluate(x=>x.complete&&x.naturalWidth===1900))throw Error('Independent image');}
...
const record={date:new Date().toISOString(),packages,questions,sharedPlatformComponents:['IntroSection','QuizBlock'],sharedStyles:'src/styles.css',checks:['48 locale/lesson navigation','152 quiz feedback checks','4 existing video metadata loads','8 independent images across lessons 1 and 2','48 mobile overflow checks','state reset','previous/next','prompt builder','no network submissions','no page errors'],humanVisualApproval:'pending'};
