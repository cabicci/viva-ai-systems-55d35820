...
parser.add_argument("--lesson",type=int,required=True,choices=range(1,13))
...
folder=BASE/f"public/generated/audio/level{args.level}"/slug/args.locale
source=json.loads((BASE/("content/level1-video.json" if args.level==1 else "content/advanced-video.json")).read_text(encoding="utf-8"))[f"{args.lesson:02}/{args.locale}" if args.level==1 else f"{args.level}/{args.lesson:02}/{args.locale}"]
...
evidence={"level":args.level,"lesson":args.lesson,"locale":args.locale,"method":"threshold 250 PCM; retain 0.45s after last speech; trim only tails over 1.5s","segments":report}
(BASE/"evidence"/f"trim-level{args.level}-{slug}-{args.locale}.json").write_text(json.dumps(evidence,indent=2)+"\n",encoding="utf-8")
