...
parser.add_argument("--lesson",required=True,type=int,choices=range(2,13))
args = parser.parse_args()
lesson_no=args.lesson
lesson_slug=f"lesson-{lesson_no:02}"
locale = args.locale
lesson=json.loads((BASE/"content/level1-video.json").read_text(encoding="utf-8"))[f"{lesson_no:02}/{locale}"]
folder = BASE/"public/generated/audio/level1"/lesson_slug/locale
...
for i, (scene, timing) in enumerate(zip(lesson["scenes"], timings)):
    ...
    assert timing["audio"] == f"generated/audio/level1/{lesson_slug}/{locale}/{i:02}.wav"
...
video = BASE/"public/generated/videos/level1"/lesson_slug/(locale+".mp4")
...
result = {"lesson":lesson_no,"locale":locale,"status":"technical-pass","frames":cursor,"durationSeconds":cursor/24,"videoSha256":hashlib.sha256(video.read_bytes()).hexdigest(),"audioSegments":evidence,"captions":"scene-aligned; source script","spokenAccuracyReview":"pending human listening"}
(BASE/"evidence").mkdir(exist_ok=True)
(BASE/"evidence"/("narrated-"+lesson_slug+"-"+locale+".json")).write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
