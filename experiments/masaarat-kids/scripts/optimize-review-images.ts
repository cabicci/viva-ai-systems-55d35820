...
for(let n=1;n<=12;n++)for(const locale of ['ar-EG','ar-MSA','ar-Gulf','en']){
 const folder=n===1?base:resolve(base,'lesson-'+String(n).padStart(2,'0'));
...
await browser.close();
console.log(JSON.stringify({images:48,pngBytes:before,webpBytes:after}));
