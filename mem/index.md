...
Lesson IDs follow `<path>-m<module#>-l<lesson#>-<slug>` (e.g. `intro-m1-l1-what-is-ai`); the older form without an explicit lesson number is deprecated. The id itself encodes path + module + order within the path — do NOT invent a global lesson number like "الدرس 22".
...
Content v2 rebuild (Intro first, then path-by-path): Historical v2 scope: the 30-lesson revision batch only, not a current global lesson-number range. Identify lessons using the mandatory path/module/order/ID format above. Rules: Tension-First (مفيش مفهوم نظري قبل ما المستخدم يحس بمشكلة) → Quick Win → مثال حسي قبل المصطلح → مصطلح تقني واحد لكل درس → Mission ≤10 دقايق وأبسط من الحالي. Metrics: boring/confusing/momentum(1-10). Target: 65-75% completion (مش 90%).
...
- [Content revision plan v2](mem://design/content-revision-plan-v2) — Full v2 doc: Tension-First + Momentum + historical 30-lesson batch scope + per-lesson workflow
