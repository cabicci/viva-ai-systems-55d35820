...
Downstream locales from MSA only. **Early Adopters**, **Feedback Loop**, **retention** preserved. Dashboard screenshot = production reference. Builder path end — bridge to other paths via `pending-path-validation`. Deferred: Bunny · Remotion · runtime.
