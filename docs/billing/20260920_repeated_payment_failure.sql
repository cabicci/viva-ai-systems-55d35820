-- Accept a second, distinct failure notification while already past_due.
-- Observed in Stripe Test Clock renewal: subscription.updated precedes invoice.payment_failed.
-- This self-transition grants no paid access. Existing ordering, ownership and event-ID guards remain.
-- Apply only after review/tests and the accepted Backup/Restore gate.
BEGIN;
